(function () {
  ...
  const debouncedSearch = debounce((raw) => {
    ...
    if (!overlay.open || overlay.kind !== "search") {
      openOverlay("search", { query: q, results: [] }, { openerFocusId: "search-input", takeFocus: false });
  ...
  function getActiveBoard() {
    ...
    if (!activeBoardId || !state.boards[activeBoardId]) activeBoardId = ids.sort((a, b) => state.boards[a].name.localeCompare(state.boards[b].name))[0];
  ...
  function renderTopbar(root) {
      ...
      el("input", {
        ...
        onKeydown: (e) => {
          if (e.key === "Enter") {
            ...
            ui.search.query = q;
            ui.search.loading = true;
            openOverlay("search", { query: q, results: [] }, { openerFocusId: "search-input", takeFocus: true });
            send({ type: "searchCards", query: q }, { trackBusy: false });
          }
      ...
      el(
        "button",
        {
          ...
          onClick: () => {
            ...
            ui.search.query = q;
            ui.search.loading = true;
            openOverlay("search", { query: q, results: [] }, { openerFocusId: "search-btn", takeFocus: true });
            send({ type: "searchCards", query: q }, { trackBusy: false });
          },
        },
        ["Search"],
    ...
    const refreshBtn = el("button", { class: "btn", onClick: () => send({ type: "refresh" }, { trackBusy: false }) }, [
      "Refresh",
    ]);

    const renameRow = (() => {
      if (!ui.renamingBoard || !b) return null;
      const input = el("input", { type: "text", value: b.name, dataset: { role: "renameBoardInput" } });
    ...
    const newBoardRow = (() => {
      ...
      const input = el("input", { type: "text", placeholder: "Board name", dataset: { role: "newBoardInput" } });
      input.addEventListener("keydown", (e) => {
        if (e.key === "Enter") {
          const name = (input.value || "").trim();
          if (!name) return;
      ...
      root.appendChild(
        el("div", { class: "banner" }, [
          el("div", {}, [
            ...
            ". ",
            el("a", { onClick: () => send({ type: "refresh" }, { trackBusy: false }) }, ["Reload latest"]),
            ".",
          ]),
        ]),
      );
	    ...
	    if (!state) {
	      boardEl.appendChild(
	        el("div", { class: "addList" }, [
	          ...
	          el("button", { class: "btn", onClick: () => send({ type: "selectRepo" }) }, ["Select repo path"]),
	        ]),
	      );
      ...
      el("div", { class: "addRow" }, [
        ...
        el(
          "button",
          {
            class: "btn primary",
            onClick: (e) => {
              const input = e.target.closest(".addList").querySelector('input[data-role="addListTitle"]');
	              const name = (input.value || "").trim();
	              if (!name) return;
	              input.value = "";
  ...
  function renderList(board, list, ordered) {
    ...
    const listEl = el("div", { class: "list", dataset: { listId: list.id } }, [
        ...
        "div",
        {
          ...
          onDrop: (e) => {
            ...
            try {
              ...
              if (insertAfter) send({ type: "moveList", boardId: board.id, listId: fromId, afterListId: targetId });
              else send({ type: "moveList", boardId: board.id, listId: fromId, beforeListId: targetId });
            } catch {}
          },
        },
      ...
      listBody,
      el("div", { class: "listFooter" }, [el("div", { class: "addRow" }, [addCardInput, addCardBtn])]),
    ]);
  ...
  function renderCard(board, list, card) {
    const due = card.dueDate ? el("span", { class: "chip", text: `Due ${formatDate(card.dueDate)}` }) : null;
    const labels = Array.isArray(card.labels) && card.labels.length > 0 ? card.labels.slice(0, 3).map((l) => el("span", { class: "chip", text: l })) : [];
    const checklist = Array.isArray(card.checklist) ? card.checklist : [];
    const done = checklist.filter((it) => it.checked).length;
    const checklistChip = checklist.length > 0 ? el("span", { class: "chip", text: `☑ ${done}/${checklist.length}` }) : null;
      ...
      "div",
      {
        ...
        onDrop: (e) => {
          ...
          try {
            ...
            if (insertAfter)
              send({ type: "moveCard", boardId: board.id, cardId: fromCardId, toListId: list.id, afterCardId: targetCardId });
            else
              send({ type: "moveCard", boardId: board.id, cardId: fromCardId, toListId: list.id, beforeCardId: targetCardId });
          } catch {}
        },
      },
      [
        el("div", { class: "cardTitle", text: card.title }),
        meta,
      ],
    );
    return cardEl;
  ...
  function renderDrawer(root) {
    ...
    const dueInput = el("input", { type: "text", value: card?.dueDate || "", placeholder: "2026-01-01T00:00:00.000Z or empty" });
    const labelsInput = el("input", { type: "text", value: (card?.labels || []).join(", "), placeholder: "labels, comma-separated" });
    const archivedToggle = el("input", { type: "checkbox", checked: !!card?.archived });
        ...
        const patch = {
          ...
          dueDate: (dueInput.value || "").trim().length === 0 ? null : (dueInput.value || "").trim(),
	    ...
	    const openPanelBtn = el("button", { class: "btn", onClick: () => card && send({ type: "openCard", cardId: card.id }, { trackBusy: false }) }, ["Open in panel"]);

    const checklistAddInput = el("input", { type: "text", placeholder: "Add checklist item…" });
	    const checklistAddBtn = el("button", {
	      class: "btn",
	      onClick: () => {
	        if (!card) return;
    ...
    commentInput.style.minHeight = "70px";
	    ...
	    const drawerEl = el("div", { class: "drawer", role: "dialog", "aria-modal": "true", "aria-labelledby": drawerTitleId }, [
	      el("div", { class: "row" }, [
	        el("div", { style: "flex:1" }, [
	          el("h2", { id: drawerTitleId, text: card ? card.title : "Card" }),
	          card ? el("div", { class: "muted", text: `cardId: ${card.id} • list: ${listName(card.listId)} • board: ${boardName(card.boardId)}` }) : el("div", { class: "muted", text: "No card selected" }),
	        ]),
      ...
      card
        ? el("div", {}, [
            ...
            el("div", { class: "field" }, [el("label", { text: "Due date (ISO or empty)" }), dueInput]),
            ...
            el("div", { class: "section" }, [
              ...
              el("div", { class: "addRow", style: "margin-bottom:10px" }, [checklistAddInput, checklistAddBtn]),
              ...checklist.map((it) =>
                el("div", { class: "checklistItem" }, [
	                  el("input", {
	                    type: "checkbox",
	                    checked: !!it.checked,
	                    onChange: (e) => send({ type: "toggleChecklistItem", cardId: card.id, itemId: it.id, checked: !!e.target.checked }),
	                  }),
	                  el("input", {
	                    type: "text",
	                    value: it.text,
	                  ...
	                  el("button", { class: "btn", onClick: () => send({ type: "removeChecklistItem", cardId: card.id, itemId: it.id }) }, ["Remove"]),
	                ]),
	              ),
	            ]),
            el("div", { class: "section" }, [
                ...
                : history
                    ...
                    .map((h) =>
                      el("div", { class: "historyItem" }, [
                        el("div", { class: "muted", text: `${formatDate(h.ts)} • ${h.actorId} • ${h.type}` }),
    ...
    if (!overlay.open) {
      ...
      return;
	    ...
	    if (overlay.kind === "search") {
	      ...
	      const panel = el("div", { class: "overlayPanel", role: "dialog", "aria-modal": "true", "aria-labelledby": panelTitleId }, [
	        el("div", { class: "row" }, [
	          el("div", { style: "flex:1" }, [el("h3", { id: panelTitleId, text: `Search: ${query}` })]),
	          el("button", { class: "btn", onClick: closeOverlay }, ["Close"]),
	        ]),
	        ...(results.length === 0
	          ? [el("div", { class: "muted", text: ui.search.loading ? "Searching…" : "No results." })]
	          : results.map((r) =>
	              el(
	                "div",
	                {
	                ...
	                [
	                  el("div", { text: r.title }),
	                  el("div", { class: "muted", text: `${boardName(r.boardId)} • ${listName(r.listId)} • ${r.cardId}` }),
	                ],
	              ),
	            )),
	      ]);
      ...
      return;
	    ...
	    if (overlay.kind === "conflicts") {
	      ...
	      const panel = el("div", { class: "overlayPanel", role: "dialog", "aria-modal": "true", "aria-labelledby": panelTitleId }, [
	        el("div", { class: "row" }, [
	          el("div", { style: "flex:1" }, [el("h3", { id: panelTitleId, text: `Conflicts (${conflicts.length})` })]),
	          el("button", { class: "btn", onClick: closeOverlay }, ["Close"]),
	        ]),
        ...(conflicts.length === 0
          ...
          : conflicts.map((c) =>
              el("div", { class: "resultRow" }, [
                ...
                el("div", { class: "muted", text: `field=${c.field} seq=${c.seq} ops=${(c.ops || []).length}` }),
              ]),
            )),
        el("div", { style: "height:10px" }),
        el("div", { class: "muted", text: "Resolve conflicts by reconciling divergent ops in the repo; this UI is read-only for conflicts in v1." }),
      ]);
      ov.appendChild(panel);
      root.appendChild(ov);
      return;
